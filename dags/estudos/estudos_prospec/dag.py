...
def prospec_1rv():
    run_prospec_on_host = BashOperator(
        ...
        bash_command=f"ssh -i /opt/airflow/config/chave-middle.pem -o StrictHostKeyChecking=no tradingenergiarz.com '{CMD_BASE} prevs NEXT-RV rodada Preliminar'",
        trigger_rule="none_failed_min_one_success",
    )
...
def prospec_ec_ext():
    run_prospec_on_host = BashOperator(
        ...
        bash_command=f"ssh -i /opt/airflow/config/chave-middle.pem -o StrictHostKeyChecking=no tradingenergiarz.com '{CMD_BASE} prevs EC-EXT rodada Definitiva'",
    )
